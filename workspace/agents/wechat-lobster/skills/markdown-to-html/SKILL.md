...
If CJK content detected, check common formatting issues:
- Bold markers with punctuation inside causing `**` parse failures
- CJK/English spacing issues

If issues detected, use `AskUserQuestion` to ask whether to fix them before conversion.

**If user agrees**: Fix formatting issues in-place, then use formatted file as input.
...
3. `publish-orchestrator` EXTEND.md `default_theme` (cross-skill fallback)
...
```bash
# Check publish-orchestrator EXTEND.md for default_theme
test -f "$HOME/.baoyu-skills/publish-orchestrator/EXTEND.md" && grep -o 'default_theme:.*' "$HOME/.baoyu-skills/publish-orchestrator/EXTEND.md"
```

```powershell
# PowerShell (Windows)
if (Test-Path "$HOME/.baoyu-skills/publish-orchestrator/EXTEND.md") { Select-String -Pattern 'default_theme:.*' -Path "$HOME/.baoyu-skills/publish-orchestrator/EXTEND.md" | ForEach-Object { $_.Matches.Value } }
```
...
|--------|-------------|---------|
| `--theme <name>` | Theme name (default, simple, modern) | default |
...
# With specific theme
${BUN_X} ${SKILL_DIR}/scripts/main.ts article.md --theme default
